...
def init_per_day(sdk):
    ...
    # 找到中证500成分股前前5天最高价和最低价的均值
    high = pd.DataFrame(sdk.getFieldData('LZ_GPA_QUOTE_THIGH')[-5:], columns=stock_list)[stock_observe]
    low = pd.DataFrame(sdk.getFieldData('LZ_GPA_QUOTE_TLOW')[-5:], columns=stock_list)[stock_observe]
    ...
    if (sdk.getNowTime() >= '093000') & (sdk.getNowTime() < '150000'):
        ...
        if removed_stock_available:
            for stock_removed in removed_stock_available:
                ...
                down_removed = down_line[stock_removed]
                if current_price_removed < down_removed:  # 判断是否卖出
        ...
        for stock in available_stock:
            ...
            elif (current_price < down) & (position > 0):
                volume = position
                order = [stock, current_price, volume, -1]

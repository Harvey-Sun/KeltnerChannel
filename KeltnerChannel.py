...
def init_per_day(sdk):
    ...
    high = pd.DataFrame(sdk.getFieldData('LZ_GPA_QUOTE_THIGH')[-5:], columns=stock_list)[zz500_not_stop]
    low = pd.DataFrame(sdk.getFieldData('LZ_GPA_QUOTE_TLOW')[-5:], columns=stock_list)[zz500_not_stop]
